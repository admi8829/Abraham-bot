...
def get_main_menu(lang="am"):
    """ዋናውን የመልዕክት ቁልፎች (Reply Keyboard) የሚሰራ"""
...
async def start_handler(message: types.Message, state: FSMContext):
    ...
    try:
        # 1. ዳታቤዝ ውስጥ መኖሩን ቼክ ማድረግ
        res = supabase.table("users").select("phone").eq("user_id", user_id).execute()
        
        # 2. አዲስ ተጠቃሚ ከሆነ ብቻ መረጃውን ማስገባት (ሪፈራልንም ጨምሮ)
        if not res.data:
            if message.text and len(message.text.split()) > 1:
                ref_arg = message.text.split()[1]
                if ref_arg.isdigit() and int(ref_arg) != user_id:
                    user_initial_data["referred_by"] = int(ref_arg)
            
            supabase.table("users").insert(user_initial_data).execute()
        else:
            # ቀድሞ ካለ እና ስልክ ቁጥር ካለው ቀጥታ ወደ ቻናል ቼክ
            if res.data[0].get('phone'):
                return await check_channel_membership(message, state)

        # 3. ስልክ ቁጥር መጠየቂያ ገጽ (ለንጹህ UI ሲባል ID እና Username አልተካተቱም)
        await state.set_state(LotteryStates.waiting_for_phone)
        
        kb = ReplyKeyboardBuilder()
        kb.row(types.KeyboardButton(text="📲 Share Contact to Register", request_contact=True))
        
        request_text = (
            f"<b>Hello {first_name},</b>\n\n"
            "To participate in our lottery and access all features, "
            "please <b>verify your identity</b> by sharing your contact number below.\n\n"
            "━━━━━━━━━━━━━━━━━━━━━\n"
            "🛡 <i>Your information is secure and will only be used for registration.</i>"
        )
...
async def show_prizes_and_pay(message: types.Message, lang: str):
    """የሽልማት ዝርዝርን ከተጠቃሚው ቋንቋ ጋር በማዛመድ ከዳታቤዝ የሚያመጣ"""
    try:
        # 1. ዳታቤዝ ውስጥ ከተጠቃሚው ቋንቋ ጋር የሚመጣጠኑ ሽልማቶችን ብቻ መምረጥ
        # .eq("lang", lang) የሚለው ተጠቃሚው 'am' ከሆነ የአማርኛውን ረድፎች ብቻ ያመጣል
        prizes_res = supabase.table("prizes").select("rank, amount").eq("lang", lang).execute()
        prizes = prizes_res.data or []
        
        prize_list = ""
        
        # 2. የቋንቋ ምርጫ መሰረት ቋሚ ጽሁፎችን ማዘጋጀት
        if lang == "am":
        ...
        if not prizes:
            ...
            for p in sorted_prizes:
                rank = p['rank']
                amount = p['amount'] # በዳታቤዝህ '10,000 ብር' ወይም '10,000 ETB' ተብሎ የተቀመጠው
                icon = "🥇" if rank == 1 else "🥈" if rank == 2 else "🥉" if rank == 3 else "🏆"
                
                if lang == "am":
                    # ምሳሌ፡ 🥇 1ኛ ደረጃ: 10,000 ብር
                    prize_list += f"{icon} <b>{rank}{rank_suffix}:</b> <code>{amount}</code>\n"
                else:
                    # ምሳሌ፡ 🥇 Rank 1: 10,000 ETB
                    prize_list += f"{icon} <b>{rank_prefix} {rank}:</b> <code>{amount}</code>\n"
        ...
        print(f"Prizes Database Error: {e}")
        error_msg = "❌ Unable to load prizes." if lang == "en" else "❌ የሽልማት ዝርዝሩን መጫን አልተቻለም።"
